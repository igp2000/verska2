...
function init() {
  ...
  }; // устанавливаем слушатели на кнопки и ссылки в модальном окне


  var add_button_listener = function add_button_listener(modal) {
    var els = modal.querySelectorAll("button, a");
    els.forEach(function (el, i) {
      set_listener(el);
    });
    els[0].focus();
    els[els.length - 1].addEventListener("keydown", function (e) {
      if (e.which === 9 && !e.shiftKey) {
        e.preventDefault();
        els[0].focus();
      }
    });
    els[0].addEventListener("keydown", function (e) {
      if (e.which === 9 && e.shiftKey) {
        e.preventDefault();
        els[els.length - 1].focus();
      }
    });
  }; // меняем стиль активной ссылки левого меню


  var set_left_menu_items_active = function set_left_menu_items_active(link) {
    class_remove(".left-menu__list-items a.active", "active");
    var links = document.querySelectorAll(".left-menu__list-items a[data-a=\"".concat(link.dataset.a, "\"]"));
  ...
  var set_listener = function set_listener(element) {
    element.addEventListener("click", function (event) {
      if (event.currentTarget.dataset.action === "left-a") {
        set_left_menu_items_active(event.currentTarget);
      } else if (event.currentTarget.dataset.action === "burger") {
        show_modal_left();
  ...
  var left_menu_items = document.querySelectorAll(".left-menu__list-items a");
  left_menu_items.forEach(function (link, i) {
    set_listener(link);
  });
