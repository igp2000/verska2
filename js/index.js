...
function init() {
  ...
  let add_button_listener = function (modal) {
    let els = modal.querySelectorAll("button, a");
    els.forEach(function (el, i) {
      set_listener(el);
    });
    els[0].focus();
  ...
  let set_left_menu_items_active = function (link) {
    class_remove(".left-menu__list-items a.active", "active");
    let links = document.querySelectorAll(
      `.left-menu__list-items a[data-a="${link.dataset.a}"]`
    );
  ...
  let left_menu_items = document.querySelectorAll(".left-menu__list-items a");
